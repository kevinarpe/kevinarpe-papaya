...
public final class FileUtil {
    ...
    // TODO: staticCheckRegularFileExists(File file)
    // TODO: staticCheckRegularFileExists(Path file)
    
    /**
     * This is a convenience method for {@link #staticCheckRegularFileExists(Path, LinkOption...)}.
     * 
    ...
    throws FileNotFoundException {
        ...
        staticCheckRegularFileExists(file, linkOptionArr);
        return file;
    }
     ...
     *         or if {@code linkOptionArr} is null or contains nulls
     * @throws FileNotFoundException if {@code file} does not exist,
     *         or if {@code file} exists but is not a regular file
     * @see #staticCheckRegularFileExists(String, LinkOption...)
     */
    public static Path staticCheckRegularFileExists(
            Path file, LinkOption... linkOptionArr)
    throws FileNotFoundException {
        ...
        try {
            ...
            String s = String.format(
                "Regular file does not exist: '%s'%nAbsolute Path: '%s'%s",
                file, file.toAbsolutePath(), _staticLinkOptionArrToString(linkOptionArr));
            throw new FileNotFoundException(s);
        }
        if (!fileAttr.isRegularFile()) {
            String s = String.format(
                "Path exists, but is not a regular file: '%s'%nAbsolute Path: '%s'%s",
                file, file.toAbsolutePath(), _staticLinkOptionArrToString(linkOptionArr));
            throw new FileNotFoundException(s);
        }
        return file;
    ...
    /**
     * This is a convenience method for {@link #staticCheckDirectoryExists(Path, LinkOption...)}.
     * 
    ...
    throws FileNotFoundException {
        ...
        staticCheckDirectoryExists(dir, linkOptionArr);
        return dir;
    }
     ...
     */
    public static Path staticCheckDirectoryExists(Path dir, LinkOption... linkOptionArr)
    throws FileNotFoundException {
        ...
        try {
            ...
            String s = String.format(
                "Directory does not exist: '%s'%nAbsolute Path: '%s'%s",
                dir, dir.toAbsolutePath(), _staticLinkOptionArrToString(linkOptionArr));
            throw new FileNotFoundException(s);
        }
        if (!fileAttr.isDirectory()) {
            String s = String.format(
                "Path exists, but is not a directory: '%s'%nAbsolute Path: '%s'%s",
                dir, dir.toAbsolutePath(), _staticLinkOptionArrToString(linkOptionArr));
            throw new FileNotFoundException(s);
        }
        return dir;
