...
public class NullablesTest {
    ...
    public void apply_Pass(@Nullable
        ...
        @Nullable
        final Integer nullableOutput = Nullables.apply(nullableInput, transform);
        Assert.assertEquals(nullableOutput, nullableExpectedOutput);
    }

    private static class _RuntimeException extends RuntimeException {}

    @Test(expectedExceptions = _RuntimeException.class)
    public void apply_FailWhenTransformThrows() {

        Nullables.apply("abc", (String s) -> { throw new _RuntimeException(); });
    }
    ...
    throws Exception {
        ...
        @Nullable
        final Integer nullableOutput = Nullables.throwingApply(nullableInput, transform);
        Assert.assertEquals(nullableOutput, nullableExpectedOutput);
    }

    private static class _Exception extends Exception {}

    @Test(expectedExceptions = _Exception.class)
    public void throwingApply_FailWhenTransformThrows()
    throws Exception {

        Nullables.throwingApply("abc", (String s) -> { throw new _Exception(); });
    }
}
